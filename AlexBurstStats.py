...
if __name__ == '__main__':
	...
	parser.add_argument('-b', '--background-rate', help='Background rate')
	parser.add_argument('-r', '--burst-rate', help='Burst Rate')
...
#############Run Ben's Burst algorithm
	#benSpans1 = run_ben_burst_alg(f, 80, 200) #background_rate, burst_rate
	#benSpans1 = run_ben_burst_alg(f, 250, 1000)
	#benSpans3 = run_ben_burst_alg(f, 500, 2000)
	#benSpans1 = run_ben_burst_alg(f, 1000, 4000)
...
#### Print Length of data run
	minTime = min(min(photons[0]['t']), min(photons[1]['t']), min(photons[2]['t']), min(photons[3]['t']))
	maxTime = max(max(photons[0]['t']), max(photons[1]['t']), max(photons[2]['t']), max(photons[3]['t']))

	lengthSeconds = (maxTime - minTime) / delta_clock

	print "Length in seconds : ", lengthSeconds

#### total photons, each stream
	photonCountDemDexc = len(photons[0])
	photonCountDemAexc = len(photons[1])
	photonCountAemDexc = len(photons[2])
	photonCountAemAexc = len(photons[3])

	photonCountDonor = photonCountDemDexc + photonCountDemAexc
	photonCountAcc = photonCountAemDexc + photonCountAemAexc

	photonCount = photonCountDonor + photonCountAcc

	print "photonCount DD ", photonCountDemDexc, " DA ", photonCountDemAexc, " AD ", photonCountAemDexc, " AA ", photonCountAemAexc, " Donor ", photonCountDonor, " Acceptor ", photonCountAcc, " Total ", photonCount

#### Print raw average of each stream, raw total

	aveDD = photonCountDemDexc / lengthSeconds
	aveDA = photonCountDemAexc / lengthSeconds
	aveAD = photonCountAemDexc / lengthSeconds
	aveAA = photonCountAemAexc / lengthSeconds
	aveDonor = photonCountDonor / lengthSeconds
	aveAcc = photonCountAcc / lengthSeconds
	aveTotal = photonCount / lengthSeconds

	print "Average Per Second DD ", aveDD, " DA ", aveDA, " AD ", aveAD, " AA ", aveAA, " Donor ", aveDonor, " Acceptor ", aveAcc, " Total ", aveTotal 

#burstParameters = [(200, 2000), (400, 2000), (300, 1500), (300, 2000), (300, 2500)]
burstParameters = [(background_rate, burst_rate)]

for bp in burstParameters:
...
#### Burst Parameters
	background_rate = bp[0]
	burst_rate = bp[1]
	print "Burst Detection Parameters Background Rate: ", background_rate, "Burst_Rate: ", burst_rate

####Plot the Alex using Ben's Burst detection
	benSpans = run_ben_burst_alg(f, background_rate, burst_rate)
...
#### Number of Bursts
	burstCount = len(Bursts_Dem_Dexc)
	print "Bursts Detected ", burstCount

#### average/stddev of burst lengths
	burstLengths = []
	totalBurstLength = 0

	for i in range(0, burstCount):
		burstLengths.append(benSpans[i][1] - benSpans[i][0])
		totalBurstLength += burstLengths[i]
		
	burstLengthMean = mean(burstLengths)
	burstLengthStd = std(burstLengths)

	print "Total Burst Length (s)", totalBurstLength / delta_clock
	print "Average Burst Length (s)", burstLengthMean / delta_clock 
	print "Std Burst Length (s)", burstLengthStd / delta_clock 

#### average/stddev of photons/burst
	DDBurstPhotonCount = []
	DABurstPhotonCount = []
	ADBurstPhotonCount = []
	AABurstPhotonCount = []
	...
	DBurstPhotonCount = []
	ABurstPhotonCount = []
	TotalBurstPhotonCount = []

	for i in range(0, burstCount):
		DDBurstPhotonCount.append(len(Bursts_Dem_Dexc[i]))
		DABurstPhotonCount.append(len(Bursts_Dem_Aexc[i]))
		ADBurstPhotonCount.append(len(Bursts_Aem_Dexc[i]))
		AABurstPhotonCount.append(len(Bursts_Aem_Aexc[i]))
		
		DBurstPhotonCount.append(len(Bursts_Dem_Dexc[i]) + len(Bursts_Dem_Aexc[i]))
		ABurstPhotonCount.append(len(Bursts_Aem_Dexc[i]) + len(Bursts_Aem_Aexc[i]))
		TotalBurstPhotonCount.append(len(Bursts_Dem_Dexc[i]) + len(Bursts_Dem_Aexc[i]) + len(Bursts_Aem_Dexc[i]) + len(Bursts_Aem_Aexc[i]))

	DDPhotonsPerBurst = mean(DDBurstPhotonCount)
	DDStdPhotonsPerBurst = std(DDBurstPhotonCount)

	DAPhotonsPerBurst = mean(DABurstPhotonCount)
	DAStdPhotonsPerBurst = std(DABurstPhotonCount)

	ADPhotonsPerBurst = mean(ADBurstPhotonCount)
	ADStdPhotonsPerBurst = std(ADBurstPhotonCount)

	AAPhotonsPerBurst = mean(AABurstPhotonCount)
	AAStdPhotonsPerBurst = std(AABurstPhotonCount)

	DPhotonsPerBurst = mean(DBurstPhotonCount)
	DStdPhotonsPerBurst = std(DBurstPhotonCount)

	APhotonsPerBurst = mean(ABurstPhotonCount)
	AStdPhotonsPerBurst = std(ABurstPhotonCount)

	TotalPhotonsPerBurst = mean(TotalBurstPhotonCount)
	TotalStdPhotonsPerBurst = std(TotalBurstPhotonCount)
		
	print "Photons Per Burst DD ", DDPhotonsPerBurst, " Std ", DDStdPhotonsPerBurst
	print "Photons Per Burst DA ", DAPhotonsPerBurst, " Std ", DAStdPhotonsPerBurst
	print "Photons Per Burst AD ", ADPhotonsPerBurst, " Std ", ADStdPhotonsPerBurst
	print "Photons Per Burst AA ", AAPhotonsPerBurst, " Std ", AAStdPhotonsPerBurst
	print "Photons Per Burst D ", DPhotonsPerBurst, " Std ", DStdPhotonsPerBurst
	print "Photons Per Burst A ", APhotonsPerBurst, " Std ", AStdPhotonsPerBurst
	print "Photons Per Burst Total ", TotalPhotonsPerBurst, " Std ", TotalStdPhotonsPerBurst

#### average/stddev of photons/sec during burst
	DDPhotonsPerSec = []
	DAPhotonsPerSec = []
	ADPhotonsPerSec = []
	AAPhotonsPerSec = []
	...
	j = 0
	for i in range(0, burstCount):
		if((benSpans[i][1] - benSpans[i][0]) > 0.0):
			DDPhotonsPerSec.append((1.0*len(Bursts_Dem_Dexc[i])) * delta_clock / (benSpans[i][1] - benSpans[i][0]))
			DAPhotonsPerSec.append((1.0*len(Bursts_Dem_Aexc[i])) * delta_clock / (benSpans[i][1] - benSpans[i][0]))
			ADPhotonsPerSec.append((1.0*len(Bursts_Aem_Dexc[i])) * delta_clock / (benSpans[i][1] - benSpans[i][0]))
			AAPhotonsPerSec.append((1.0*len(Bursts_Aem_Aexc[i])) * delta_clock / (benSpans[i][1] - benSpans[i][0]))
			
			DPhotonsPerSec.append(DDPhotonsPerSec[j] + DAPhotonsPerSec[j])
			APhotonsPerSec.append(ADPhotonsPerSec[j] + AAPhotonsPerSec[j])
			TotalPhotonsPerSec.append(DDPhotonsPerSec[j] + DAPhotonsPerSec[j] + ADPhotonsPerSec[j] + AAPhotonsPerSec[j])
			j += 1
		
	meanDDPhotonsPerSec = mean(DDPhotonsPerSec)
	stdDDPhotonsPerSec = std(DDPhotonsPerSec)

	meanDAPhotonsPerSec = mean(DAPhotonsPerSec)
	stdDAPhotonsPerSec = std(DAPhotonsPerSec)

	meanADPhotonsPerSec = mean(ADPhotonsPerSec)
	stdADPhotonsPerSec = std(ADPhotonsPerSec)

	meanAAPhotonsPerSec = mean(AAPhotonsPerSec)
	stdAAPhotonsPerSec = std(AAPhotonsPerSec)

	meanDPhotonsPerSec = mean(DPhotonsPerSec)
	stdDPhotonsPerSec = std(DPhotonsPerSec)

	meanAPhotonsPerSec = mean(APhotonsPerSec)
	stdAPhotonsPerSec = std(APhotonsPerSec)

	meanTotalPhotonsPerSec = mean(TotalPhotonsPerSec)
	stdTotalPhotonsPerSec = std(TotalPhotonsPerSec)

	print "Mean Photons Per Sec DD ", meanDDPhotonsPerSec, " Std ", stdDDPhotonsPerSec
	print "Mean Photons Per Sec DA ", meanDAPhotonsPerSec, " Std ", stdDAPhotonsPerSec
	print "Mean Photons Per Sec AD ", meanADPhotonsPerSec, " Std ", stdADPhotonsPerSec
	print "Mean Photons Per Sec AA ", meanAAPhotonsPerSec, " Std ", stdAAPhotonsPerSec
	
	print "Mean Photons Per Sec Donor ", meanDPhotonsPerSec, " Std ", stdDPhotonsPerSec
	print "Mean Photons Per Sec Acceptor ", meanAPhotonsPerSec, " Std ", stdAPhotonsPerSec
	print "Mean Photons Per Sec Total ", meanTotalPhotonsPerSec, " Std ", stdTotalPhotonsPerSec


#### average/stddev of photons/sec during noise
	totalNoiseTime = lengthSeconds - totalBurstLength / delta_clock

	totalDDNoisePhotons = photonCountDemDexc - sum(DDBurstPhotonCount)
	totalDANoisePhotons = photonCountDemAexc - sum(DABurstPhotonCount)
	totalADNoisePhotons = photonCountAemDexc - sum(ADBurstPhotonCount)
	totalAANoisePhotons = photonCountAemAexc - sum(AABurstPhotonCount)
	
	totalDNoisePhotons = totalDDNoisePhotons + totalDANoisePhotons
	totalANoisePhotons = totalADNoisePhotons + totalAANoisePhotons

	totalNoisePhotons = totalDNoisePhotons + totalANoisePhotons
	
	noiseAverageDD = totalDDNoisePhotons / totalNoiseTime
	noiseAverageDA = totalDANoisePhotons / totalNoiseTime
	noiseAverageAD = totalADNoisePhotons / totalNoiseTime
	noiseAverageAA = totalAANoisePhotons / totalNoiseTime

	noiseAverageD = totalDNoisePhotons / totalNoiseTime
	noiseAverageA = totalANoisePhotons / totalNoiseTime
	noiseAverageTotal = totalNoisePhotons / totalNoiseTime

	print "Mean Photons in Noise DD ", noiseAverageDD
	print "Mean Photons in Noise DA ", noiseAverageDA
	print "Mean Photons in Noise AD ", noiseAverageAD
	print "Mean Photons in Noise AA ", noiseAverageAA

	print "Mean Photons in Noise D ", noiseAverageD
	print "Mean Photons in Noise A ", noiseAverageA
	print "Mean Photons in Noise Total ", noiseAverageTotal

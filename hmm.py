...
def fisher_score( hmm, Obs ):
    T = len( Obs )
    O = symbol_index( hmm, Obs )
    log_Prob_Obs, Alpha, c = forward( hmm, Obs )
    Beta = backward( hmm, Obs, c )
    Gamma_raw = Alpha * Beta
    Gamma = Gamma_raw / Gamma_raw.sum( 0 )
#    Theta_raw = Alpha[ :,1: ] * Beta[ :,T-1 ]
#    Theta = Theta_raw / Theta_raw.sum( 0 )
    
    fisher_score_emits = numpy.zeros( [ hmm.N, hmm.M ] )
#    fisher_score_trans = numpy.zeros( [ hmm.N, hmm.N ] )
    
    for t in xrange( T ):
        v = O[ t ]
        fisher_score_emits[ :,v ] += Gamma[ :,t ] 
##         if t < T-1:
##             normalizer = sum( Theta[ :,t ]  * hmm.B[ :, v ]  * A[ :,j ] )
##             for j in xrange( hmm.M ):
##                 fisher_score_trans[ :,j ] += Theta[ :,t ] * hmm.B[ :,v ] * A[ :j ]  * 1.0 / normalizer
    
    
    return fisher_score_emits.flatten()
    
def norm_df( X, mu=0, sigma=1.0 ):
    """
    ...
    # evaluate for univariate samples
    if len( numpy.shape( X ) ) < 2 or numpy.shape( X )[ 1 ] == 1:
        try:
            return 1.0 / ( numpy.sqrt( 2 * numpy.pi ) ) * sigma * numpy.exp( -( X - mu )**2 / ( 2 * sigma**2 ) )
        except OverflowError:
            if len( numpy.shape( X ) ) < 2:
                return 10**( -300 )
            else:
                df_vals = numpy.zeros( numpy.shape( X ), float )
                for i in xrange( numpy.shape( X )[ 0 ] ):
                    try:
                        df_vals[ i,: ] = 1.0 / ( numpy.sqrt( 2 * numpy.pi ) ) * sigma * numpy.exp( -( X[ i ] - mu )**2 / ( 2 * sigma**2 ) )
                    except:
                        df_vals[ i,: ] = 0#10**( -300 )
                return df_vals
    
    # evaluate multivariate samples
    else:
        X = numpy.matrix( X )
        N, D = numpy.shape( X )
        
        if mu == 0:
            mu = numpy.matrix( numpy.zeros( [ D ], float ).reshape( ( 1, D ) ) )
        if sigma == 1.0:
            sigma = numpy.matrix( numpy.identity( D ) )
    
        normConstant = 1.0 / ( 2 * numpy.pi )**( D/2 ) * numpy.sqrt( linalg.det( sigma ) )        
        invSigma = numpy.matrix( linalg.inv( sigma ) )
        
        df_vals = normConstant * numpy.exp( -1.0 / 2 * ( X[ 0,: ] - mu )  * invSigma * ( X[ 0,: ] - mu  ).T )
        
        for i in xrange( 1,N ):
            df_vals = numpy.vstack( [ df_vals, normConstant * numpy.exp( -1.0 / 2 * ( X[ i,: ] - mu )  * invSigma * ( X[ i,: ] - mu  ).T ) ] )
            
        return numpy.array( df_vals )
    ...
    def __init__( self, n_states=1, **args ):
        ...
        if 'V' in args:
            ...
            # assume univariate
                ...
                self.D = 1
                #raise "You need to include dimensionality of observations: D=D in arg list"
            
            self.V = 'Real-valued, %s-dimensional' % ( self.D )
            self.M = 1            
        ...
        if 'B' in args:
            ...
            # define make_B function for continuous observations
            else:
                self.B_params = args[ 'B' ]
                
                assert len( self.B_params ) == self.N
                def make_B( Obs ):
                    self.B = numpy.zeros( [ self.N, numpy.shape( Obs )[ 0 ] ], float )
                    for i in xrange( self.N ):
                        self.B[ i,: ] = norm_df( Obs, mu=self.B_params[ i ][ 0 ], sigma=self.B_params[ i ][ 1 ] ).T
                self.make_B = make_B
            ...
            if self.type == "discrete":
                ...
                raise "You Must Have Mu and Sigma List for This Version"
                # TODO: initialize to normal distributions
...
def forward( hmm, Obs, scaling=True ):
    ...
    for t in xrange( 1,T ):
        if hmm.type == "discrete":
            ...
            Alpha[ :,t ] = numpy.dot( Alpha[ :,t-1 ], hmm.A) * hmm.B[ :,t  ]
        if scaling:
...
def backward( hmm, Obs, c=None ):
    ...
    for t in reversed( xrange( T-1 ) ):
        if hmm.type == "discrete":
            ...
            Beta[ :,t ] = numpy.dot( hmm.A,  ( hmm.B[ :,t+1  ] * Beta[ :,t+1 ] ) )
        if c is not None:
            Beta[ :,t ] = Beta[ :,t ] * c[ t ]
...
def viterbi( hmm, Obs, scaling=True ):
    ...
    if scaling:
        if hmm.type == "discrete":
            ...
            Delta[ :,0 ] = numpy.log( hmm.Pi ) + numpy.log( hmm.B[ :,0 ] )

    else:
        if hmm.type == "discrete":
            Delta[ :,0 ] = hmm.Pi * hmm.B[ :,Obs[ 0] ]
        else:
            Delta[ :,0 ] = hmm.Pi * hmm.B[ :,0 ]
    ...
    if scaling:
        for t in xrange( 1,T ):
            ...
            if hmm.type == "discrete":
                Delta[ :,t ] = nus.max( 0 ) + numpy.log( hmm.B[ :,Obs[ t ] ] )
            else:
                Delta[ :,t ] = nus.max( 0 ) + numpy.log( hmm.B[ :,t  ] )
            Psi[ :,t ] = nus.argmax( 0 )
    else:
        for t in xrange( 1,T ):
            nus =  Delta[ :,t-1 ] * hmm.A
            if hmm.type == "discrete":
                Delta[ :,t ] = nus.max( 0 ) * hmm.B[ :,Obs[ t ] ]
            else:
                Delta[ :,t ] = nus.max( 0 ) * hmm.B[ :,t ]
            Psi[ :,t ] = nus.argmax( 0 )
...
def baum_welch( hmm, Obs_seqs, **args ): 
    ...
    updatePi=updateMean=updateVar=updateA=updateB=scaling=graph = 1
    normUpdate=verbose=validating = 0
    if 'updatePi' in args: updatePi = args[ 'updatePi' ]
    if 'updateMean' in args: updateMean = args[ 'updateMean' ]
    if 'updateVar' in args: updateVar = args[ 'updateVar' ]
    ...
    for epoch in xrange( epochs ):
        ...
        for Obs in Obs_seqs:
            ...
            for t in xrange( T-1 ):
                for i in xrange( hmm.N ):
                    if hmm.type == "discrete":
                        ...
                        Xi[ i,:,t ] = Alpha[ i,t ] * hmm.A[ i,: ] * hmm.B[ :,t+1 ] * Beta[ :,t+1 ]
            ...
            if hmm.type == "discrete" and updateB:
        ...
        if hmm.type == "discrete":
            if updateB:
                # reestimate emission probabilites ( observable symbol probability distribution )
                for i in xrange( hmm.N ):
            ...
            if updateMean:
                Mu_bar = numpy.array( numpy.matrix( Gamma ) * numpy.matrix( Obs ) )
                Mu_bar = Mu_bar.T / Gamma.sum( 1 )
                Mu_bar = Mu_bar.flatten().tolist()

            else:
                Mu_bar = []
                for set in hmm.B_params:
                    Mu_bar.append( set[ 0 ] )

            if updateVar:
                Sigma_bar = numpy.zeros( [ hmm.N, hmm.D ] )

                for j in xrange( hmm.N ):              
                    Sigma_bar[ j,: ] = numpy.matrix( Gamma[ j,: ] ) * numpy.matrix( ( Obs - hmm.B[ j ] [ 0 ] ) * ( Obs - hmm.B[ j ] [ 0 ] ))  / numpy.sum( Gamma[ j,: ] )

                Sigma_bar = Sigma_bar.flatten().tolist()
            else:
                Sigma_bar = []
                for set in hmm.B_params:
                    Sigma_bar.append( set[ 1 ] )

            hmm.B_params =  zip( Mu_bar, Sigma_bar )
...
def dishonest_casino_test( graph = True ):
    ...
    A = numpy.array( [ [ 0.99, 0.01 ], \
                       [ 0.01, 0.99 ] ] )
    state_1_data = rand.normal( -1,.1, 500).reshape( (500,1 ) )
    state_2_data = rand.normal( 1,.1, 500).reshape( (500,1 ) )
    Obs = numpy.vstack( [ state_1_data, state_2_data ] )
    hmm = HMM( 2, D=1, B=B, A=A )
    hmm.make_B( Obs )
    #hmm_train( hmm=hmm, Obs=Obs, epochs = 20, graph=1 )
